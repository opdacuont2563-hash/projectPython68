...
def _load_env():
    try:
        ...
        if meipass and (Path(meipass) / ".env").exists():
            load_dotenv(dotenv_path=Path(meipass) / ".env")
    except Exception:
        pass
...
API_HOST = os.environ.get("SURGIBOT_API_HOST", "0.0.0.0")
API_PORT = int(os.environ.get("SURGIBOT_API_PORT", "8088"))

# รอบประกาศเสียง (นาที) — server จะ sync ค่านี้ไปชีต "Config"
ANNOUNCE_MIN = int(os.environ.get("SURGIBOT_ANNOUNCE_MINUTES", "20"))
...
AUTO_DELETE_AFTER_DISCHARGE_MIN = int(os.environ.get("SURGIBOT_AUTO_DELETE_MIN", "3"))
...
SPREADSHEET_ID = os.environ.get("SURGIBOT_SPREADSHEET_ID", "1dr6pCw8dEnCh_UYJXJzAFsthdZ8IsRBF3VlNw1AlvjI")
...
def _load_service_account_credentials():
    ...
    """
    env_json = os.environ.get("SURGIBOT_GCP_CREDENTIALS_JSON", "").strip()
    env_file = os.environ.get("SURGIBOT_GCP_CREDENTIALS_FILE", "").strip()
    embedded = os.environ.get("SURGIBOT_EMBEDDED_CREDENTIALS_JSON", "").strip()
...
def init_sheets():
    ...
    try:
        ...
        SHEETS_ENABLED = True
        print("[Sheets] Connected and enabled.")
    except Exception as e:
        SHEETS_ENABLED = False
        _gspread_client = None
        _sheet = None
        print(f"[Sheets] Disabled (reason: {e}). Running without Sheets.")
...
def sync_config_to_sheet():
    ...
    try:
        ...
        print(f"[Sheets] Config sync error: {e}")
...
def _update_next_announce_to_sheet(next_dt: datetime):
    ...
    try:
        ...
        print(f"[Sheets] update next announce error: {e}")
...
class SurgeryStatusApp(tk.Frame):
    ...
    # --- TTS helpers ---
    def _tts_play_blocking(self, text: str, filename: str, lang: str, max_wait_s: int = 120):
        """เล่นเสียงจนจบแบบ blocking (ใช้ในเธรดแยก)"""
        try:
    ...
    def _speak_bilingual_async(self, th_text: str, en_text: str, pause_ms: int = BILINGUAL_PAUSE_MS):
        ...
        def _runner():
            with self._audio_lock:
                self._tts_play_blocking(th_text, filename="speak_th.mp3", lang="th")
                time.sleep(max(0, pause_ms) / 1000.0)
                self._tts_play_blocking(en_text, filename="speak_en.mp3", lang="en")
        ...
        def _runner():
            ...
            with self._audio_lock:
                for i in range(POSTPONED_REPEAT):
                    self._tts_play_blocking(th_msg, filename="postponed_th.mp3", lang="th")
                    time.sleep(BILINGUAL_PAUSE_MS / 1000.0)
                    self._tts_play_blocking(en_msg, filename="postponed_en.mp3", lang="en")
    ...
    def _apply_status_change(self, patient_id, new_status, eta_minutes=None, announce=True):
        ...
        try:
            ...
            print("[Sheets] sync after status change error:", e)
    ...
    def delete_selected(self):
        ...
        if removed > 0:
            try:
                ...
                print("[Sheets] sync after delete error:", e)
            update_snapshot_from_dict(self.patient_data)
    ...
    def update_timers(self):
        ...
        if to_delete:
            ...
            try:
                ...
                print("[Sheets] sync after auto-delete error:", e)
            update_snapshot_from_dict(self.patient_data)
...
def schedule_next_public_announcement(app_self: SurgeryStatusApp):
    def do_announce():
        ...
        try:
            ...
            print("[Sheets] next announce write error:", e)
    ...
    try:
        ...
        print("[Sheets] first next announce write error:", e)
